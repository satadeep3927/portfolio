...
const OUTPUT_PDF = path.join(__dirname, '../public/resume/Satadeep_Dasgupta_CV.pdf');
...
function checkPandoc() {
  return new Promise((resolve, reject) => {
    exec('pandoc --version', (error, stdout, stderr) => {
      if (error) {
        reject(new Error('Pandoc is not installed. Please install Pandoc first:\n' +
  ...
  return new Promise((resolve, reject) => {
    ...
    exec(command, (error, stdout, stderr) => {
      if (error) {
        ...
        convertViaPuppeteer(inputFile, outputFile)
          .then(resolve)
          .catch(reject);
      } else {
